 ...
 * depth (not a CSS transform trick), with a slow idle drift when the
 * pointer isn't over it. Returns a cleanup function, or null if the
 * scene never started (no WebGL, reduced motion, or the image failed to
 * load) — the plain <img> underneath stays visible in that case (see the
 * `.about__photo-wrap.is-3d` toggle in the caller).
 *
 * Deferred by design: the caller only invokes this once the About
 * section actually scrolls into view — this is well below the fold, so
 * there's no reason to fetch three.js or spend a GPU frame on it any
 * earlier than that.
 *
 * Everything below this point touches a real WebGL context and an image
 * decode, neither of which jsdom implements — tests/about-scene.test.ts
 * covers the fallback paths only, same reasoning as hero-scene.ts.
 */
/* v8 ignore start */
export async function initAboutScene(canvas: HTMLCanvasElement, imageUrl: string): Promise<AboutSceneCleanup | null> {
  ...
  // Skip rendering work when scrolled off-screen or the tab is
  // backgrounded — same battery/GPU discipline as the hero scene.
  let isVisible = true;
