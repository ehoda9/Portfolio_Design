...
if ($StartDocker) {
    ...
    for ($i = 0; $i -lt 30; $i++) {
        try {
            Invoke-RestMethod -Uri "$ApiBase/api/health" -Method Get -TimeoutSec 2 | Out-Null
            $ready = $true
            break
        }
        catch {
    ...
    if (-not $ready) { Write-Fail "API did not become ready within 60 seconds."; exit 1 }
    Write-Ok "API is up."
}
...
try {
    $createResponse = Invoke-RestMethod -Uri "$ApiBase/api/posts" -Method Post -ContentType "application/json" `
        -Headers @{ Authorization = "Bearer $token" } -Body $postBody
...
$listResponse = Invoke-RestMethod -Uri "$ApiBase/api/posts" -Method Get
$listResponse.posts | Format-Table slug, title, publishedAt -AutoSize
